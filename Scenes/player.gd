...
var blood_particles_scene = preload("res://Scenes/blood_particles.tscn")
...
func take_damage(amount: int):
	...
	blood_particles.get_node("Particles").emitting = true
	AudioManagerSingleton.play_sfx(load("res://Audio/LosingHealth.wav"))
...
func _on_shoot_timer_timeout():
	...
	for i in manual_projectile_count:
		...
		if is_horizontal:
			offset.y = spacing * relative
		else:
			offset.x = spacing * relative
...
func _update_xp_bar():
	if has_node("/root/Main/HUD/ExpBackground/ExpFill"):
...
func start_regeneration():
	if regen_timer == null:
		...
		regen_timer.wait_time = max(regen_timer.wait_time - 0.5, 1.0)
